...
def test_set_cookie(client_factory, monkeypatch):
...
def test_expires_on_set_cookie(client_factory, monkeypatch, expires):
...
async def test_streaming_response_stops_if_receiving_http_disconnect():
    ...
    with anyio.move_on_after(1) as cancel_scope:
        await response({"method": "GET"}, receive_disconnect, send)
